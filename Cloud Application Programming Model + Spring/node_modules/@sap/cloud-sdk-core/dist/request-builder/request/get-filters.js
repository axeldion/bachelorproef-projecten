...
function getODataFilterExpression(filter, parentFieldNames, targetEntityConstructor) {
    ...
    if (filter_1.isFilterLink(filter)) {
        var linkExp = filter.filters
            .map(function (subFilter) { return getODataFilterExpression(subFilter, __spreadArrays(parentFieldNames, [filter.link._fieldName]), filter.link._linkedEntity); })
            .filter(function (f) { return !!f; })
            .join(' and ');
    ...
    if (filter_1.isFilter(filter)) {
        if (typeof filter.field === 'string') {
            ...
            return [__spreadArrays(parentFieldNames, [filter.field]).join('/'), filter.operator, value].join(' ');
        }
        else {
