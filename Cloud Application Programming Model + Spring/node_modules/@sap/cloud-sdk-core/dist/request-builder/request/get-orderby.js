...
function getODataOrderByExpressions(orderBys, parentFieldNames) {
    ...
    return orderBys.reduce(function (expressions, orderBy) {
        if (orderBy instanceof order_1.OrderLink) {
            return __spreadArrays(expressions, [getOrderByExpressionForOrderLink(orderBy, __spreadArrays(parentFieldNames))]);
        }
        return __spreadArrays(expressions, [getOrderByExpressionForOrder(orderBy, parentFieldNames)]);
    }, []);
}
function getOrderByExpressionForOrderLink(orderBy, parentFieldNames) {
    if (parentFieldNames === void 0) { parentFieldNames = []; }
    return getODataOrderByExpressions(orderBy.orderBy, __spreadArrays(parentFieldNames, [orderBy.link._fieldName])).join(',');
}
function getOrderByExpressionForOrder(orderBy, parentFieldNames) {
    if (parentFieldNames === void 0) { parentFieldNames = []; }
    return [__spreadArrays(parentFieldNames, [orderBy._fieldName]).join('/'), orderBy.orderType].join(' ');
}
