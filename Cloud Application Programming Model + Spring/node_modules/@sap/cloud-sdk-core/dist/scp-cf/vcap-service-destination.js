...
function destinationForServiceBinding(serviceInstanceName, options) {
    ...
    var destination = options.transformationFn ? options.transformationFn(selected) : transform(selected);
    if (destination && proxy_util_1.proxyStrategy(destination) === proxy_util_1.ProxyStrategy.INTERNET_PROXY) {
