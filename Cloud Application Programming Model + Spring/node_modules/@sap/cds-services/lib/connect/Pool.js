...
class Pool {
  ...
  _acquireFromPool () {
    return this._pool.acquire().catch(() => {
      ...
      throw new Error(
        `Requesting client from pool timed out. Max pool size is ${options.max}, current pool size is ${
          this._pool.size
        }. Increase 'acquireTimeoutMillis' for longer waiting time or 'max' to create more clients.`
      )
    })
  }
