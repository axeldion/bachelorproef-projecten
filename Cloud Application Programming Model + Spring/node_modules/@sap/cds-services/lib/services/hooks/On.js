...
class On extends Base {
  ...
  _replaceQueryRef (context, name, localizedEntityName) {
    ...
    if (
      ...
      this._replaceWhereRef(
        context.query.SELECT.where,
        name,
        context.query.SELECT.from.as || context.query.SELECT.from.ref[0]
      )
    }
  }
  ...
  _checkForLocalizedEntity (event, context, handler) {
    if (
      ...
      (context.target['@fiori.draft.enabled'] || !isDraftEnabled(this._model.definitions, context.target.name))
    ) {
      let locale = ''
