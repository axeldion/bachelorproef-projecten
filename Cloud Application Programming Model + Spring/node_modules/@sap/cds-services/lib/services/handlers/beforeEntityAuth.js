...
const _valuesToOrConditions = (columnRef, op, values) => {
  ...
  for (const value of values) {
    ...
    arr.push(_addSymbol(columnRefCopy), op, _addSymbol({ val: value }))
  }
...
const _xprToCondition = (context, columnRef, op, userRef) => {
  ...
  return [columnRef, op, _addSymbol({ val: attrValue })]
}
...
const _parseWhereFromXpr = (context, xpr) => {
  ...
  while (i < length) {
    ...
    if (xpr[i].ref && _isString(xpr[i + 1]) && xpr[i + 2].ref) {
      if (xpr[i].ref[0] === '$user') {
...
const _parseWhere = (annotatedWhere, context) => {
  let { xpr } = cds.parse.expr(annotatedWhere)
