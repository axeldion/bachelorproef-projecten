...
const _getCurrent = async context => {
  ...
  if (!current) {
    current = checkExistCQN ? await context.run(checkExistCQN) : []
  }
...
const onUpdate = () => async context => {
   ...
   * else -> do nothing
   */
  const current = await _getCurrent(context)
  if (current.length === 0) {
    ...
    const count = await context.run(cqn)
