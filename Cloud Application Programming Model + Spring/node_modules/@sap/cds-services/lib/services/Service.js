...
class Service extends Client {
  ...
  async _calculateDiff (context, currentData) {
    const {
      data,
      event,
      run,
    ...
    if (event === 'UPDATE' && context.target['@cds.persistence.skip'] !== true) {
      if (context.run) {
        try {
      ...
      const combinedData =
        currentData || Object.assign({}, context.query.UPDATE.data || {}, context.query.UPDATE.with || {})
      return compareJson(combinedData, context._.partialPersistentState, target)
    }

    if (event === 'PATCH') {
      if (context.run) {
        // SELECT.one because req.query in custom handler does not have access to _drafts
        context._.partialPersistentState = await context.run(
          SELECT.from(context.target.name)
            .where(this._createWhereCondition(target, data))
            .limit(1)
        )
